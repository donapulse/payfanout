...
- SDK errors surface as `processing_error` with the SDK's error on `raw`. One delivered
  through the buttons' `onError` is **not retryable**: PayPal documents that callback as a
  catch-all with nothing to handle beyond a generic error message or page. A failed
  button render stays retryable, since mounting again can succeed.
