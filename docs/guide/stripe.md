...
| `requestTimeoutMs` | - | SDK default (`80000`) | Abort a hung Stripe request, response body included; surfaces as a retryable `psp_unavailable`. |
| `maxNetworkRetries` | - | `2` | Network-level retries inside the Stripe SDK. Calls that create objects or move money carry the caller's idempotency key, so a retry cannot duplicate them. The SDK also replays a closed connection once, even at `0`. |
