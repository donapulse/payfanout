...
| `PAYPAL_NEGATIVE_TESTING` | optional | Set to `1` to run the `INSTRUMENT_DECLINED` mock-response case (REST calls need only the `PayPal-Mock-Response` header; the account-level Negative Testing setting is documented for the Classic APIs) |
