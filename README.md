...
`merchantRefNum` instead of replaying the original, reads a write whose answer was lost
back by that reference and re-sends a payment, capture or refund only after a 429); and
`withRetry(fn, policy)` from
...
(`requestTimeoutMs`, default 60s per exchange, the response timeout of Paysafe's own SDKs):
a hung read surfaces as a retryable `psp_unavailable` instead of hanging your request
handler. A write that times out is looked up by its `merchantRefNum` instead of being
re-sent; a payment, capture or refund whose original cannot be read back fails with a
non-retryable `processing_error`, to retry later with the same idempotency key.
