...
export interface StripeServerAdapterConfig {
   ...
   * milliseconds (the SDK's `timeout` client option). Applies per attempt: a
   * timeout before the response starts is retried within `maxNetworkRetries`.
   * Left unset, the SDK's own default of 80000 (80s) applies. Only applies
   * when the SDK is loaded lazily — an injected client keeps its own
   * configuration.
   */
  requestTimeoutMs?: number;
  /**
   * Automatic network-level retries inside the Stripe SDK: an integer >= 0,
   * default 2. Calls that create objects or move money carry the caller's
   * idempotency key, so a retry cannot duplicate them; at 0 the SDK still
   * retries a closed connection once. Only applies when the SDK is loaded
